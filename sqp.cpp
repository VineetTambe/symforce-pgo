...
template <typename T>
bool SQP<T>::run_solve_qp(const Matrix& P, const Vector& q, const Matrix& A, const Vector& l, const Vector& u,
                          Vector& prim, Vector& dual)
{
  // ----------------------------------------------

  //   ProxQP specific init params
  double eps_abs = 1e-9;
  isize dim = P.rows(), n_eq = 0, n_in = A.rows();
  ...
  // ‘Eigen::SparseMatrix<double, 0, int>*’ to
  // ‘proxsuite::optional<Eigen::SparseMatrix<double, 0, int> >’
  // vector<vector<>>
  //   T = double;
  //   Eigen::MatrixXf H = &P;

  Eigen::SparseMatrix<T> H_spa(n_in, dim);
  H_spa = P.sparseView();
  //   proxsuite::optional<Eigen::SparseMatrix<double>> H_opt = proxsuite::optional::make_optional(*H_spa);

  //   Eigen::MatrixXf C = &A;
  Eigen::SparseMatrix<T> C_spa(n_in, dim);
  C_spa = A.sparseView();
  //   proxsuite::optional<Eigen::SparseMatrix<double, 0, int>> C_opt = proxsuite::optional::make_optional(C_spa);
  Eigen::Matrix<T, Eigen::Dynamic, 1> g = Eigen::Matrix<T, Eigen::Dynamic, 1>(dim, 1);
  g = q;

  Eigen::Matrix<T, Eigen::Dynamic, 1> l_prime = Eigen::Matrix<T, Eigen::Dynamic, 1>(n_in);
  l_prime = l;

  Eigen::Matrix<T, Eigen::Dynamic, 1> u_prime = Eigen::Matrix<T, Eigen::Dynamic, 1>(n_in);
  u_prime = u;

  auto optional_H_spa = std::make_optional(H_spa);
  auto optional_C_spa = std::make_optional(C_spa);

  auto optional_g = std::make_optional(g);

  auto optional_l = std::make_optional(l_prime);

  auto optional_u = std::make_optional(u_prime);

  qp.init(optional_H_spa, optional_g, nullopt, nullopt, optional_C_spa, optional_l, optional_u);
  qp.solve();
  ...
  info_.qp_solver_iter += qp.results.info.iter;
  ...
  if (qp.results.info.status == QPSolverOutput::PROXQP_MAX_ITER_REACHED)
  {
  ...
  // std::cout << "ProxQP Primal res: " << qp.results.info.pri_res << " | ProxQP Dual res: " << qp.results.info.dua_res
  //           << std::endl;
  // std::cout << "OSQP Primal op: " << qp_solver_.primal_solution() << " | OSQP Dual op: " <<
  // qp_solver_.dual_solution()
  //           << std::endl;
  prim = qp.results.x;
  dual << qp.results.y, qp.results.z;
  // std::cout << "Primal op: " << prim << " | Dual res: " << dual << std::endl;

  // Todo initialize dual with proxqp y and z

  // prim = Eigen::Matrix<float, Eigen::Dynamic, 1>::Constant(qp.results.info.pri_res);
  // dual = Eigen::Matrix<float, Eigen::Dynamic, 1>::Constant(qp.results.info.dua_res);
